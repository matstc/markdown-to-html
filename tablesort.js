...
;(function () {
    ...
    Tablesort.prototype = {
        ...
        sortTable: function(header, update) {
            ...
            var isUsefulItem = function(item){
                return item.substr(0, 4) !== '<!--' && item.length !== 0;
            };

            while (items.length < 2 && i < t.tBodies[0].rows.length) {
                item = trim(getInnerText(t.tBodies[0].rows[i].cells[column]));
                if (isUsefulItem(item)) items.push(item);
                i++;
            }

            i = t.tBodies[0].rows.length - 1;
            while (items.length < 4 && i >= 0) {
                item = trim(getInnerText(t.tBodies[0].rows[i].cells[column]));
                if (isUsefulItem(item)) items.push(item);
                i--;
            }
    ...
    var testNumber = function(number){
        return number.match(/^-?[£\x24Û¢´€] ?\d/) || // prefixed currency
          number.match(/^-?\d+\s*[€]/) || // suffixed currencty
          number.match(/^-?(\d+[,\.]?)+(E[\-+][\d]+)?%?$/); // number
    };
